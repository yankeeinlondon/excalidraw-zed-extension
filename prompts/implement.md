...
start:
    message: "🎬  starting the implementation of phase **#{{phase}}** of `{{parent_dir(plan)}}` (**agent:** {{ctx.agent}}/{{ctx.model}})"
success: 
    say: "Phase {{phase}} of the plan was implemented successfully"
    message: "{{pass_icon}}  phase **{{phase}}** (_of {{total_phases}}_) of the plan `{{parent_dir(plan)}}` successfully completed ({{ctx.agent}}/{{ctx.model}})"
...
failure:
    say: "Phase {{phase}} of a plan `{{parent_dir(plan)}}`, ran into problems!"
    message: "❌️  phase **{{phase}}** (_of {{total_phases}}_) failed in the plan `{{parent_dir(plan)}}` ({{ctx.agent}}/{{ctx.model}}: {{err.msg}})"
    effect: sad-trombone
loop:
...
- all tests are passing (using `just test`)
- all tests meet the design requirements for testing (stated above)
- all lints are passing (using `just lint`)
