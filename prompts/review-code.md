...
- look for tests which use unsafe data to test:
    - OS-specific assumptions embedded (we expect this to work on macOS, Linux, and Windows)
        - if a particular test requires OS specific semantics then you must ensure that each OS has the same coverage
        - either using an OS-aware based input to the test or having a OS-specific test block
    - using the codebase as a test bed rather than creating a fixture
...
- Save the following frontmatter properties to the review file (@{{review}}):
    ...
    - set the `reviewed_by` property to "{{ctx.agent}}/{{ctx.model}}" 
